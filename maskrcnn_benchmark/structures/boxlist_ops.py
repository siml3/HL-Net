...
def boxlist_nms_set(boxlist, set_ids, nms_thresh, max_proposals=-1, score_field="scores"):
    """
    Performs non-maximum suppression on a boxlist, with scores specified
    in a boxlist field via score_field.
    ...
    boxes = boxlist.bbox
    score = boxlist.get_field(score_field)
    dets = torch.cat((boxes, score[:, None], set_ids[:, None].to(score)), dim=-1).cpu().numpy()
    # keep = _box_nms(boxes, score, nms_thresh)
    keep = cython_nms.nms_set(dets, nms_thresh)
    device = boxes.device
...
def cat_boxlist_nofield(bboxes):
    """
    Concatenates a list of BoxList (having the same image size) into a
    single BoxList
