...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_statistics_new(self):
        fg_matrix, bg_matrix = get_VG_statistics_new(img_dir=self.img_dir, roidb_file=self.roidb_file, dict_file=self.dict_file,
                                                image_file=self.image_file, must_overlap=True)
        eps = 1e-3
        # bg_matrix += 1
        fg_matrix[:, :, 0] = bg_matrix
        ...
        pred_dist = torch.nn.functional.log_softmax(
            torch.from_numpy(pred_dist), dim=-1
        ).numpy().astype(fg_matrix.dtype)
