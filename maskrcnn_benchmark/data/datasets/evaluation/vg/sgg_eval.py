...
class SGRecall(SceneGraphEvaluation):
    def __init__(self, config, result_dict):
        super(SGRecall, self).__init__(result_dict)
        self.thr_s = config.TEST.RELATION.PRE_NMS_PREDICTION_THRES
        self.thr_p = config.TEST.RELATION.LATER_NMS_PREDICTION_THRES
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        if self.pred is False:
            pred_rels, pred_scores = rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, self.thr_p)
        else:
            pred_rels, pred_scores = rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, self.thr_s)
...
""" 
def rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, nms_thresh=0.3):
    ...
    is_overlap = (rel_ious>=nms_thresh)&(sub_labels[:,None]==sub_labels[None,:])&(obj_labels[:,None]==obj_labels[None,:])
