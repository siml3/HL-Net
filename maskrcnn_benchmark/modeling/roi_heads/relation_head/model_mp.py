...
class ART_block(nn.Module):
    ...
    def forward(self, obj_feats, phr_feats, pair_idxs, sign_list, pairs_list):
        refined_obj_feats = []
        atten_map_list = []
        if pairs_list is None:
            pairs_list = [None] * len(obj_feats)
        for iobj_feats, iphr_feats, ipair_idxs, ipairs, isign in zip(
                obj_feats, phr_feats, pair_idxs, pairs_list, sign_list
        ):
        ...
        return refined_obj_feats, atten_map_list
...
class ARTs(nn.Module):
    ...
    def forward(self, roi_features, proposals, union_features, pair_idxs, logger=None):
        ...
        for i in range(self.mp_layer_num):
            obj_feats, atten_maps = self.obj_mps[i](obj_feats, phr_feats, pair_idxs, sign_list, subsets_list)
            outputs = [ioutputs + iobj_feats * self.temp[i + 1].to(iobj_feats) for ioutputs, iobj_feats in
        ...
        return obj_dists, obj_preds, obj_feats, atten_map_list, aux_loss
