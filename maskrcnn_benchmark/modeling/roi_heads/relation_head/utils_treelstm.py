...
class BiTreeLSTM_Foreward(nn.Module):
    ...
    def forward(self, tree, features, treelstm_io):
        ...
        if tree.left_child is None:
            ...
            # print('(l:{})->{}'.format(tree.left_child.index, tree.index))
            # assert tree.left_child.index == tree.index + 1
        ...
        if tree.right_child is None:
            ...
            # print('(r:{})->{}'.format(tree.right_child.index, tree.index))
            # assert tree.right_child.index == tree.index + 1
        # Only being used in decoder network
        if self.is_pass_embed: 
...
class BiTreeLSTM_Backward(nn.Module):
    ...
    def forward(self, tree, features, treelstm_io):
        ...
        if tree.parent is None:
            ...
            # print('p:{}->{}'.format(tree.parent.index, tree.index))
            # assert tree.parent.index + 1 == tree.index
