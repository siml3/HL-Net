...
def obj_prediction_set_nms(boxes_per_cls, set_id, pred_logits, nms_thresh=0.3):
    """
    boxes_per_cls:               [num_obj, num_cls, 4]
    pred_logits:                 [num_obj, num_category]
    """
    ...
    is_not_same_set = (set_id[:, None] != set_id[None, :]).cpu().numpy()
    ...
    for i in range(num_obj):
        ...
        if float(pred_label[int(box_ind)]) > 0:
            ...
            pred_label[int(box_ind)] = int(cls_ind)
        prob_sampled[np.logical_and(is_overlap[box_ind,:,cls_ind], is_not_same_set[box_ind, :]), cls_ind] = 0.0
        prob_sampled[box_ind] = -1.0 # This way we won't re-sample
