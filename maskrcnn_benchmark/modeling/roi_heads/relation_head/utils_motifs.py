...
def sort_by_att_score(proposals):
    num_rois = [len(b) for b in proposals]
    num_im = len(num_rois)
    att_scores = [p.extra_fields['attention_map'] for p in proposals]
    assert num_rois == [int(att.shape[0]) for att in att_scores]
    
    sort_rois_idx = list(np.argsort(-np.array(num_rois)))
    head_id = np.cumsum([0] + num_rois)
    inv_head_id = np.cumsum([0] + sorted([n**2 for n in num_rois], reverse=True))
    sort_idx_sets = []
    inv_perm_sets = []
    for i, idx in enumerate(sort_rois_idx):
        sort_idx = (att_scores[idx] + torch.eye(
            num_rois[idx]
            ).to(att_scores[idx]) * 10).argsort(-1, descending=True)
        sort_idx += int(head_id[idx])
        sort_idx_sets.append(sort_idx.view(-1))

        inv_perm_i = int(inv_head_id[i]) + torch.arange(
            num_rois[idx]).to(att_scores[idx]).long() * num_rois[idx]

        inv_perm_sets.append(inv_perm_i)

    inv_perm_sets = [inv_perm_sets[i] for i in list(
        np.argsort(sort_rois_idx))]
    
    sort_idx_sets = torch.cat(sort_idx_sets, 0)
    inv_perm_sets = torch.cat(inv_perm_sets, 0)

    # batch_size = []
    new_num_rois = []
    for num_roi in sorted(num_rois, reverse=True):
        new_num_rois += [num_roi] * num_roi
    
    _, batch_size = transpose_packed_sequence_inds(new_num_rois)

    batch_size = torch.LongTensor(batch_size)

    # head_id = torch.LongTensor(head_id).to(att_scores[0].device)

    return sort_idx_sets, inv_perm_sets, batch_size
