    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ("HLNetPredictor","HLCausalAnalysisPredictor"):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ("HLNetPredictor","HLCausalAnalysisPredictor"):
            ...
            refine_logits, relation_logits, add_losses = self.predictor(proposals, rel_pair_idxs, full_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, features, self.union_feature_extractor, logger)
        else:
