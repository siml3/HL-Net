...
class RelationFeatureExtractor(nn.Module):
    ...
    def forward_bbox_pairs(self, x, head_proposals, tail_proposals):
        device = x[0].device
        union_proposals = []
        rect_inputs = []
        for head_proposal, tail_proposal in zip(head_proposals, tail_proposals):
            if head_proposal is None or tail_proposal is None:
                continue
            union_proposal = boxlist_union(head_proposal, tail_proposal)
            union_proposals.append(union_proposal)

            # use range to construct rectangle, sized (rect_size, rect_size)
            num_rel = len(head_proposal)
        ...
        if len(union_proposals) == 0:
            return torch.empty((0, self.out_channels)).to(x[0])
