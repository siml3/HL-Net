...
def rebuild_forest(input_forest, proposals):
    output_forest = []
    for tree, proposal in zip(input_forest, proposals):
        output_forest.append(rebuild_tree(tree, proposal))

    return output_forest

def rebuild_tree(input_tree, proposal):
    node_list = []
    # flag = True
    node = input_tree
    # node_list.append(node)
    obj_labels = proposal.get_field("predict_logits")[:, 1:].max(-1)[1] + 1

    def depth_tree(node, node_list):
        node_list.append(node)
        if node.left_child is not None:
            depth_tree(node.left_child, node_list)
        if node.right_child is not None:
            depth_tree(node.right_child, node_list)

    depth_tree(node, node_list)
    # while flag:
    #     if node.left_child is not None:
    ...
    #         flag = False
    index = [node.index for node in node_list]
    # index = [int(obj_labels[node.index]) for node in node_list]
    sort_idx = list(np.argsort(np.array(index)))
    # if len(sort_idx) > 1:
    #     tmp = sort_idx[0]
    #     sort_idx[:-1] = sort_idx[1:]
    #     sort_idx[-1] = tmp
    # sort_node_list = []
    root = None
    for i, ind in enumerate(sort_idx):
        if i == 0:
            node_list[ind].is_root = True
            node_list[ind].left_child = node_list[sort_idx[i+1]] if i < len(sort_idx) - 1 else None
            node_list[ind].right_child = None
            node_list[ind].parent = None
            node_list[ind].num_child = 1
            node_list[ind].vaild_label = int(obj_labels[node_list[ind].index])
            if i < len(sort_idx) - 1:
                node_list[ind].left_child.parent = node_list[ind]
            root = node_list[ind]
        elif i < len(sort_idx) - 1:
            node_list[ind].is_root = False
            node_list[ind].right_child = node_list[sort_idx[i+1]]
            node_list[ind].left_child = None
            node_list[ind].num_child = 1
            node_list[ind].right_child.parent = node_list[ind]
            node_list[ind].vaild_label = int(obj_labels[node_list[ind].index])
        else:
            node_list[ind].is_root = False
            node_list[ind].right_child = None
            node_list[ind].left_child = None
            node_list[ind].num_child = 0
            node_list[ind].vaild_label = int(obj_labels[node_list[ind].index])
...
def generate_forest(pair_scores, proposals, mode):
    ...
    for pair_score, proposal in zip(pair_scores, proposals):
        ...
        obj_confidence = F.softmax(proposal.get_field("predict_logits"), dim=-1)[:, 1:].max(-1)[0]
        sort_idx = obj_confidence.sort(descending=True)[1].tolist()
