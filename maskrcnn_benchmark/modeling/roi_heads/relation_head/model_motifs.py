...
from .utils_motifs import obj_edge_vectors, center_x, sort_by_score, to_onehot, get_dropout_mask, nms_overlaps, encode_box_info, sort_by_att_score
import ipdb
import dill as pkl
...
class LSTMContextSplitArrange(LSTMContext):

    def __init__(self, *args, **kwargs):
        super(LSTMContextSplitArrange, self).__init__(*args, **kwargs)
    ...
    def obj_ctx(self, obj_feats, proposals, obj_labels=None, boxes_per_cls=None, ctx_average=False):
        ...
        # Sort by the confidence of the maximum detection.
        # ipdb.set_trace()
        perm_single, inv_perm_single, ls_transposed_single = self.sort_rois(proposals)
        
        perm, inv_perm, ls_transposed = sort_by_att_score(proposals)
        ...
        if self.mode != 'predcls':
            ...
            obj_dists, obj_preds = self.decoder_rnn(
                ...
                labels=obj_labels[perm] if obj_labels is not None else None,
                boxes_for_nms=boxes_per_cls[perm] if boxes_per_cls is not None else None, 
                # TODO: it may have problems in sgdet.
                )
            obj_preds = obj_preds[inv_perm]
            obj_dists = obj_dists[inv_perm]
        else:
        ...
        return obj_dists, obj_preds, encoder_rep, perm_single, inv_perm_single, ls_transposed_single