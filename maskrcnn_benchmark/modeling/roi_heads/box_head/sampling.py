...
from maskrcnn_benchmark.modeling.matcher import Matcher, Matcher_EMD
...
class FastRCNNSampling_EMD(FastRCNNSampling):
    """
    Sampling RoIs
    """
    def __init__(
        ...
        super(FastRCNNSampling_EMD, self).__init__(proposal_matcher, fg_bg_sampler, box_coder)
    ...
    def match_targets_to_proposals(self, proposal, target):
        ...
        #     assert matched_idxs.max() < target.bbox.shape[0]
        matched_targets = target[matched_idxs.clamp(min=0)]
        matched_targets.add_field("matched_idxs", matched_idxs)
        return matched_targets
    ...
    def subsample(self, proposals, targets):
        ...
        labels_sampling = [label[:, 0] for label in labels]

        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels_sampling)

        # assert len(sampled_pos_inds) > 0
        ...
        # concatenated via the fg_bg_sampler, into individual feature map levels
        for img_idx, (pos_inds_img, neg_inds_img) in enumerate(zip(sampled_pos_inds, sampled_neg_inds)):
            img_sampled_inds = torch.nonzero(pos_inds_img | neg_inds_img).view(-1)
            # if len(img_sampled_inds) > 0:
            #     assert img_sampled_inds.max() < proposals[img_idx].bbox.shape[0]
            proposals_per_image = proposals[img_idx][img_sampled_inds]
            proposals[img_idx] = proposals_per_image
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            # assert matched_idxs_per_image.shape == labels_per_image.shape
            ...
            if targets_per_image.bbox.shape[0] == 1:
                labels_per_image[:, -1] = -1

            # compute regression targets
            # assert matched_targets.bbox.shape[0] == proposals_per_image.bbox.shape[0]
            regression_targets_per_image0 = self.box_coder.encode(
                matched_targets.bbox[:, 0], proposals_per_image.bbox
            )
            regression_targets_per_image1 = self.box_coder.encode(
                matched_targets.bbox[:, 1], proposals_per_image.bbox
            )
            regression_targets_per_image = torch.stack((regression_targets_per_image0, 
                                            regression_targets_per_image1), dim=1)
...
def make_roi_box_samp_processor_emd(cfg):
    matcher = Matcher_EMD(
        cfg.MODEL.ROI_HEADS.FG_IOU_THRESHOLD,
        cfg.MODEL.ROI_HEADS.BG_IOU_THRESHOLD,
        allow_low_quality_matches=False,
    ...
    samp_processor = FastRCNNSampling_EMD(
        matcher,
        fg_bg_sampler,
        box_coder,
    )
