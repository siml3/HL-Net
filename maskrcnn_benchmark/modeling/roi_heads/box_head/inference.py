...
from maskrcnn_benchmark.structures.boxlist_ops import boxlist_nms, boxlist_nms_set
...
class PostProcessor_EMD(PostProcessor):
    def __init__(
        self,
        score_thresh=0.05,
        nms=0.5,
        ...
        super(PostProcessor_EMD, self).__init__(score_thresh,
        nms,
    ...
    def add_important_fields(self, i, boxes, orig_inds, boxlist, boxes_per_cls, relation_mode=False):
        if relation_mode:
            if not self.custum_eval:
                assert int(orig_inds.max()) < len(boxes[i])*2
                # gt_labels = boxes[i].get_field('labels').view(-1)[orig_inds]
                # gt_attributes = boxes[i].get_field('attributes').view(-1,
                #     boxes[i].get_field('attributes').shape[-1])[orig_inds]
                #
                # boxlist.add_field('labels', gt_labels)
                # boxlist.add_field('attributes', gt_attributes)

            predict_logits = boxes[i].get_field('predict_logits').view(-1,
                boxes[i].get_field('predict_logits').shape[-1])[orig_inds]
            boxlist.add_field('boxes_per_cls', boxes_per_cls)
            boxlist.add_field('predict_logits', predict_logits)
    ...
    def forward(self, x, boxes, relation_mode=False):
        ...
        Returns:
            results (list[BoxList]): one BoxList for each image, containing
                the extra fields labels and scores
        """
        features, class_logits0, class_logits1, box_regression0, box_regression1 = x
        num_cls = class_logits0.shape[-1]
        dim_reg = box_regression0.shape[-1]
        class_logits = torch.stack((class_logits0, class_logits1), dim=1).contiguous().view(-1, num_cls)
        box_regression = torch.stack((box_regression0, box_regression1), dim=1).contiguous().view(-1, dim_reg)
        class_prob = F.softmax(class_logits, -1)

        # TODO think about a representation of batch of boxes
        image_shapes = [box.size for box in boxes]
        # boxes_per_image = [len(box) for box in boxes]
        boxes_per_image_time2 = [len(box)*2 for box in boxes]
        concat_boxes = torch.cat([a.bbox for a in boxes], dim=0)
        concat_boxes = concat_boxes.unsqueeze(1).repeat(1, 2, 1).contiguous().view(sum(boxes_per_image_time2), -1)

        if self.cls_agnostic_bbox_reg:
            box_regression = box_regression[:, -4:]
        # add rpn regression offset to the original proposals
        proposals = self.box_coder.decode(
            box_regression.view(sum(boxes_per_image_time2), -1), concat_boxes
        ) # tensor of size (num_box, 4*num_cls)
        if self.cls_agnostic_bbox_reg:
            proposals = proposals.repeat(1, class_prob.shape[1])
        ...
        features = features.unsqueeze(1).repeat(1, 2, 1).contiguous().view(-1, features.shape[-1])
        features = features.split(boxes_per_image_time2, dim=0)
        # proposals = proposals.unsqueeze(1).repeat(1, 2, 1).contiguous().view(-1, proposals.shape[-1])
        proposals = proposals.split(boxes_per_image_time2, dim=0) # [2*N, 4*num_cls]
        class_prob = class_prob.split(boxes_per_image_time2, dim=0) # [2*N, 4*num_cls]
        ...
        for i, (prob, boxes_per_img, image_shape) in enumerate(zip(
            ...
            assert boxes_per_img.shape[0] % 2 == 0
            boxlist = self.prepare_boxlist(boxes_per_img, prob, image_shape)
            boxlist = boxlist.clip_to_image(remove_empty=False)
            assert self.bbox_aug_enabled == False
            if not self.bbox_aug_enabled:  # If bbox aug is enabled, we will do it later
                boxlist, orig_inds, boxes_per_cls, set_id = self.filter_results(boxlist, num_classes)
            # add 
            boxlist = self.add_important_fields(i, boxes, orig_inds, boxlist, boxes_per_cls, relation_mode)
            boxlist.add_field('set_id', set_id)
    ...
    def filter_results(self, boxlist, num_classes):
        ...
        result = []
        orig_inds = []
        set_id = torch.arange(boxes.shape[0] // 2).unsqueeze(1).repeat(1, 2).contiguous().view(-1).to(boxes)
        # Apply threshold on detection probabilities and apply NMS
        # Skip j = 0, because it's the background class
        inds_all = scores > self.score_thresh
        for j in range(1, num_classes):
            inds = inds_all[:, j].nonzero().squeeze(1)
            scores_j = scores[inds, j]
            set_id_j = set_id[inds]
            boxes_j = boxes[inds, j * 4 : (j + 1) * 4]
            boxlist_for_class = BoxList(boxes_j, boxlist.size, mode="xyxy")
            boxlist_for_class.add_field("pred_scores", scores_j)
            boxlist_for_class, keep = boxlist_nms_set(
                boxlist_for_class, set_id_j, self.nms, max_proposals=self.post_nms_per_cls_topn, score_field='pred_scores'
            )
            inds = inds[keep]
            num_labels = len(boxlist_for_class)
            boxlist_for_class.add_field(
        ...
        if number_of_detections > self.detections_per_img > 0:
            ...
            result = result[keep]
            orig_inds = orig_inds[keep]
        return result, orig_inds, boxes_per_cls[orig_inds], set_id[orig_inds]
...
def make_roi_box_post_processor_emd(cfg):
    use_fpn = cfg.MODEL.ROI_HEADS.USE_FPN
    ...
    postprocessor = PostProcessor_EMD(
        score_thresh,
        nms_thresh,
        post_nms_per_cls_topn,
        nms_filter_duplicates,
